...
# (p.ej. ./corrigeAlumno.sh desde el directorio que contiene src lib include)
#
#   DICHO DIRECTORIO NO CONTENDRA NINGUN ARCHIVO $EJE (p.ej. ya que en este fichero EJE=practica1,
#   en ese directorio no podra haber ningun fichero que se llame "practica1")
#
#   DICHO DIRECTORIO CONTENDRA UN FICHERO makefile QUE GENERE EL EJECUTABLE $EJE CUYO PROGRAMA
#   PRINCIPAL SERA $MAIN (p.ej. ya que en este fichero EJE=practica1, el makefile ha de crear
#   un ejecutable de nombre "practica1", como el makefile mostrado en la siguiente seccion )
#
...
if test -e $NOMTMPOK; then
  rm $NOMTMPOK
fi

# Elimina el temporal que cuenta los oks
if test -e corrigeUno.log; then
  rm corrigeUno.log
fi

# Elimina el temporal que cuenta los oks
if test -e corrigeUnoBD.log; then
  rm corrigeUnoBD.log
fi

# Elimina el temporal que cuenta los oks
if test -e corrigeUno.res; then
  rm corrigeUno.res
fi

function testOK() {
  if test -e $DIRALU/$EJE; then
