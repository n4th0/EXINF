...
public:
  ...
  unordered_map<string, InformacionTerminoPregunta> &getIndicePregunta() {
...
private:
  ...
  // un indexador sin inicializarlo convenientemente. La inicialización la
  // decidirá el alumno
  ...
  string pregunta = "";
  // Pregunta indexada actualmente. Si no hay ninguna indexada, contendría el
  // valor ""
  ...
  // Palabras de parada. El filtrado de palabras de parada se realizará, tanto
  // en la pregunta como en los documentos, teniendo en cuenta el parámetro
  // minuscSinAcentos y tipoStemmer. Es decir que se aplicará el mismo proceso a
  // las palabras de parada almacenadas en el fichero antes de realizar el
  // filtrado (p.ej. si se aplica el pasar a minúsculas los términos del
  // documento/pregunta a indexar, para comprobar si se ha de eliminar el
  // término, éste se comparará con la versión de palabras de parada en
  // minúsculas). Esto se pide así para casos en los que en el
